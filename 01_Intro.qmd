...
In this first lecture, we will introduce R and RStudio (the software we will be using in this course). We will then move on to using R as a calculator and saving simple objects of different data types.
...
R provides many built-in functions (also sometimes called "commands"). We have already lerned about two functions: `ls()`, which lists all objects currently in the environment, and `str()`, which tells us the structure - including data type - of an object. You can think of a function as an instruction to R - in the example of `str()`, that instruction is "take the object in the brackets and tell us about its data type and structure". You can even write your own functions for code you want to reuse often (covered on the last day).
